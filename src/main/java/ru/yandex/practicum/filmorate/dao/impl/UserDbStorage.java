...
public class UserDbStorage implements UserStorage {
    ...
    private static final String SQL_GET_USER_ID = "SELECT user_id FROM users WHERE email = ? AND login =  ? AND " +
            "name = ? AND birthday = ?";
    private static final String SQL_GET_UPDATE_USER = "UPDATE users SET email = ?,  login = ?, name = ?, " +
    ...
    public User updateUser(User user) {
        jdbcTemplate.update(SQL_GET_UPDATE_USER, user.getEmail(), user.getLogin(), user.getName(),
                user.getBirthday(), user.getId());
        log.debug("Данные пользователя {} обновлены.", user.getId());
        return user;
    }
    ...
    public Integer removeUser(Integer id) {
        ...
        log.debug("Ползователь {} удален.", id);
        return id;
    }
    ...
    @Override
    public Integer removeFriends(Integer id, Integer friendId) {
    ...
    public Collection<User> getMutualFriends(Integer id, Integer friendId) {
        return friendDao.getMutualFriends(id, friendId);
    }
